...
-- FIXED VERSION WITHOUT EXPLICIT COMMITS
-- ========================================
-- Set delimiter for trigger definitions
DELIMITER // -- Drop existing triggers if they exist
DROP TRIGGER IF EXISTS after_article_insert // DROP TRIGGER IF EXISTS after_article_status_update // DROP TRIGGER IF EXISTS after_user_registration // DROP TRIGGER IF EXISTS after_comment_insert // DROP TRIGGER IF EXISTS after_reaction_insert // DROP TRIGGER IF EXISTS article_audit_trigger // -- ========================================
...
AFTER
UPDATE ON articles FOR EACH ROW BEGIN -- Log the change
...
-- Note: If indexes already exist, these statements will produce warnings but won't fail
-- Indexes for audit_log table
-- DROP INDEX IF EXISTS idx_audit_log_table_record ON audit_log;
-- DROP INDEX IF EXISTS idx_audit_log_created_at ON audit_log;
-- CREATE INDEX idx_audit_log_table_record ON audit_log(table_name, record_id);
-- CREATE INDEX idx_audit_log_created_at ON audit_log(created_at);
-- Indexes for admin_notifications table  
-- DROP INDEX IF EXISTS idx_admin_notifications_type ON admin_notifications;
-- DROP INDEX IF EXISTS idx_admin_notifications_read ON admin_notifications;
...
-- Indexes should be created separately using the indexes.sql file.